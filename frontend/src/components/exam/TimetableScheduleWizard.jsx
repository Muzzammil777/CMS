...
export default function TimetableScheduleWizard({ isOpen, onClose, onSave }) {
  ...
  const inputClasses = "w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#4c1d95]/10 focus:border-[#4c1d95] outline-none transition-all text-sm bg-white text-slate-800";
            ...
            return (
              <div key={idx} className="flex flex-col items-center flex-1 relative">
                <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors z-10 ${
                  isActive || isPassed ? 'bg-[#4c1d95] text-white' : 'bg-slate-100 text-slate-400'
                }`}>
                  {isPassed ? '✓' : stepNum}
                </div>
                ...
                {idx < 2 && (
                  <div className={`absolute top-3.5 sm:top-4 left-[50%] w-full h-[2px] z-0 transition-colors ${isPassed ? 'bg-[#4c1d95]' : 'bg-slate-100'}`} />
                )}
              </div>
            );
          })}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1.5 bg-[#4c1d95]/10 hover:bg-[#4c1d95]/20 text-[#4c1d95] rounded-lg text-xs font-semibold transition-all"
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-[#4c1d95]/10 focus:border-[#4c1d95] outline-none"
                        />
                      </div>
                      <div>
            ...
            <div className="space-y-5">
              ...
              {loadingPreview ? (
                <div className="text-center py-16 text-slate-500">
                  <span className="material-symbols-outlined text-4xl animate-spin text-[#4c1d95] mb-2">sync</span>
                        ...
                        {previewExams.map((exam, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/55 transition-colors">
                            <td className="px-5 py-4">
                              <span className="text-[10px] font-bold text-[#4c1d95] block uppercase">{exam.code}</span>
                            ...
                            <td className="px-5 py-4 font-semibold text-slate-700">
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-[#4c1d95]/5 text-[#4c1d95] border border-[#4c1d95]/10">
                      ...
                      <div key={idx} className="p-4 border border-slate-200 bg-white rounded-xl shadow-sm space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs border-t border-slate-100 pt-2.5">
                          ...
                          <div>
                            ...
                            <span className="inline-flex items-center gap-1 mt-0.5 text-xs font-semibold text-[#4c1d95]">
          ...
          <div className="flex gap-2">
            {wizardStep < 3 ? (
              <button
                type="button"
                onClick={handleNext}
                className="px-5 py-2.5 bg-[#4c1d95] hover:bg-[#3b0764] text-white rounded-lg text-sm font-semibold transition-all active:scale-95 shadow-sm"
              >
                Next
              </button>
            ) : (
              <button
                ...
                className="px-5 py-2.5 bg-[#4c1d95] hover:bg-[#3b0764] text-white rounded-lg text-sm font-semibold transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
