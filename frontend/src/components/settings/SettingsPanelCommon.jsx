...
export const inputCls =
  'w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#6d28d9]/20 focus:border-[#6d28d9] outline-none transition-all text-sm text-slate-700 bg-white';
  ...
  return (
    <div className="flex items-center justify-end gap-3 mt-6 pt-5 border-t border-slate-100">
      ...
      <button
        ...
        className="px-6 py-2 bg-[#6d28d9] hover:bg-[#4c1d95] text-white text-sm font-semibold rounded-lg transition-all shadow-sm active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
      >
...
export function SettingsLoader({ label = 'Loading…' }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-10 flex items-center justify-center gap-3 text-slate-400 text-sm">
      <span className="inline-block w-4 h-4 rounded-full border-2 border-slate-200 border-t-[#6d28d9] animate-spin" />
      {label}
    </div>
  );
}
...
export function ToggleRow({ label, description, checked, onChange }) {
  return (
    <label className="flex items-center justify-between gap-4 py-3 border-b border-slate-50 last:border-0 cursor-pointer group">
      <div>
        <p className="text-sm font-semibold text-slate-800 group-hover:text-[#6d28d9] transition-colors">{label}</p>
      ...
      <div className="relative flex-shrink-0">
        ...
        />
        <div className="w-10 h-5 bg-slate-200 peer-checked:bg-[#6d28d9] rounded-full transition-colors duration-200" />
