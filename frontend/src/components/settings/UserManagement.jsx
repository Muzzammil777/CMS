        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 w-full bg-white border border-slate-200 rounded-lg text-sm text-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#4c1d95]/30 focus:border-[#4c1d95] transition-all"
            />
          </div>
        </div>
                ...
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <label className="relative inline-flex items-center cursor-pointer gap-2">
                      ...
                      />
                      <div className="w-8 h-4 bg-slate-200 peer-checked:bg-[#4c1d95] rounded-full transition-colors" />
                      ...
                      <span className={`text-xs font-medium ${u.active ? 'text-[#4c1d95]' : 'text-slate-400'}`}>{u.active ? 'Active' : 'Disabled'}</span>
                    </label>
                  </td>
                  <td className="px-5 py-3">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-[#4c1d95]/10 rounded-lg">
                  <span className="material-symbols-outlined text-[#4c1d95]">{activeUser.id ? 'edit' : 'person_add'}</span>
                </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input type="checkbox" checked={activeUser.active} onChange={(e) => setActiveUser((cur) => ({ ...cur, active: e.target.checked }))} className="w-4 h-4 accent-[#4c1d95]" />
            ...
            <div className="p-5 border-t border-slate-200 flex items-center justify-end gap-3">
              ...
              <button onClick={saveUser} className="px-6 py-2 bg-[#4c1d95] hover:bg-[#3b0764] text-white text-sm font-semibold rounded-lg transition-all shadow-sm active:scale-95">
