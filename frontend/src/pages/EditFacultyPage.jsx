...
export default function EditFacultyPage() {
  ...
  const fetchFaculty = async () => {
    try {
      setLoading(true);
      const res = await fetch(buildApiUrl(`/faculty/${encodeURIComponent(id)}`));
      ...
      setFormData({
        ...
        email: data.email || '',
        phone: data.phone || '',
        dateOfBirth: formatDateForInput(data.dateOfBirth || data.dob),
        gender: data.gender || '',
        role: data.role || data.designation || 'Assistant Professor',
        department: data.department || data.department_id || '',
        yearsOfExperience: data.yearsOfExperience?.toString() || data.years_of_experience?.toString() || '0',
        highestQualification: data.highestQualification || data.qualification || '',
        specialization: data.specialization || (Array.isArray(data.specializations) ? data.specializations[0] : '') || '',
        university: data.university || '',
        employmentType: data.employmentType || data.employment_type || 'Full-Time',
        status: data.status || data.employment_status || 'Active',
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/faculty/${encodeURIComponent(id)}`), {
        method: 'PUT',
      ...
      navigate(`/faculty/${encodeURIComponent(id)}`);
    } catch (err) {
  ...
  const steps = [
    { id: 1, title: 'Personal', icon: 'person', helpText: 'Update full name, email, DOB, and phone details.' },
    { id: 2, title: 'Professional', icon: 'work', helpText: 'Update faculty designation, department, and years of experience.' },
    { id: 3, title: 'Qualification', icon: 'school', helpText: 'Update degree qualification, specialization, and university.' },
    { id: 4, title: 'Employment', icon: 'badge', helpText: 'Update employment type and status.' },
    { id: 5, title: 'Review', icon: 'rate_review', helpText: 'Review faculty details before saving changes.' },
  ];
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-600 font-semibold text-sm">Loading Faculty Details...</p>
        </div>
      </div>
    );
  }
  ...
  const completionPercentage = (filledCount / 7) * 100;

  return (
    <EnterpriseWizardTemplate
      title="Edit Faculty Profile"
      subtitle={`Update details and credentials for ${formData.fullName || 'Faculty'}`}
      steps={steps}
    ...
    >
      ...
      {currentStep === 2 && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-[#5F6B7A] uppercase tracking-wider block mb-1">Designation / Role</label>
              <select
                name="role"
                value={formData.role}
                onChange={handleInputChange}
          ...
          <div className="bg-white p-5 rounded-2xl border border-[#E6EDF2] shadow-sm space-y-4">
            <div className="flex items-center gap-2 text-[#7C3AED] border-b border-[#F1F5F9] pb-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#64748B] font-medium">Update degree qualifications, university details, and academic rank</p>
              </div>
            </div>
      ...
      {currentStep === 4 && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-[#5F6B7A] uppercase tracking-wider block mb-1">Employment Type</label>
              <select
                name="employmentType"
            ...
            <div>
              <label className="text-xs font-bold text-[#5F6B7A] uppercase tracking-wider block mb-1">Status</label>
              <select
                name="status"
      ...
      {currentStep === 5 && (
        <div className="space-y-4">
          <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
            <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Faculty Profile Summary</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
              <div><span className="text-slate-500">Name:</span> <span className="font-bold text-slate-800">{formData.fullName}</span></div>
              <div><span className="text-slate-500">Designation:</span> <span className="font-bold text-slate-800">{formData.role}</span></div>
              <div><span className="text-[#5F6B7A]">Department:</span> <span className="font-bold text-slate-800">{formData.department}</span></div>
              <div><span className="text-slate-500">Degree:</span> <span className="font-bold text-slate-800">{formData.highestQualification}</span></div>
              <div><span className="text-slate-500">Experience:</span> <span className="font-bold text-slate-800">{formData.yearsOfExperience} yrs</span></div>
              <div><span className="text-slate-500">Type:</span> <span className="font-bold text-slate-800">{formData.employmentType}</span></div>
            </div>
          </div>
        </div>
      )}
