...
// ─── Category metadata ────────────────────────────────────────────────────────
const CAT_META = {
  academic:   { color: '#6d28d9', bg: '#eff6ff', border: '#bfdbfe', emoji: '', label: 'Academic'},
  ...
  meetings:   { color: '#8b5cf6', bg: '#ecfeff', border: '#a5f3fc', emoji: '', label: 'Meetings'},
  ...
  scholarship:{ color: '#4c1d95', bg: '#eff6ff', border: '#bfdbfe', emoji: '', label: 'Scholarship' },
};
...
// ─── Styles ───────────────────────────────────────────────────────────────────
const pill = (active, color = '#6d28d9') =>({
    ...
    return (
      <div
        ...
        style={{
          ...
          border: `1.5px solid ${isExp ? '#6d28d9' : n.read ? '#f3f4f6' : '#bfdbfe'}`,
          borderRadius: 14,
          padding: '16px 18px',
          cursor: 'pointer',
          transition: 'all 0.2s',
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginTop: n.pinned ? 8 : 0 }}>{/* Category icon bubble */}
          <div style={{
            ...
            {isExp && (
              ...
              >{!n.read && (
                  <button type="button" onClick={() =>markRead(n.id)} style={actionBtn('#6d28d9')}><Icon.Check />Mark as Read
                  </button>)}
        ...
        <main className={`main-content${isSidebarVisible ? '' : ' sidebar-hidden'} min-w-0 w-full overflow-x-hidden`}>{/* Topbar */}
          ...
          <div className="content-card" style={{ marginBottom: 20, padding: '14px 18px' }}><div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>{/* Search */}
              ...
              {unreadCount >0 && (
                <button type="button" onClick={markAllRead} style={{
                  ...
                  color: '#6d28d9', fontSize: 13, fontWeight: 600, cursor: 'pointer',
          ...
          {visible.length === 0 && (
            <div className="content-card" style={{ textAlign: 'center', padding: '60px 24px' }}><div style={{ fontSize: 48, marginBottom: 16 }}>{searchQuery ? '' : showArchived ? '' : ''}
                ...
                <button type="button" onClick={() =>setSearchQuery('')} style={{ marginTop: 16, ...actionBtn('#6d28d9', '#eff6ff'), fontSize: 13 }}>Clear Search
                </button>)}
            </div>)}
